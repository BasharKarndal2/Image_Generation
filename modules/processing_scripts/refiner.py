...
class ScriptRefiner(scripts.ScriptBuiltinUI):
    ...
    def ui(self, is_img2img):
        with InputAccordion(False, label="المكرر", elem_id=self.elem_id("enable")) as enable_refiner:
            with gr.Row():
                refiner_checkpoint = gr.Dropdown(label='أختر النموذج', elem_id=self.elem_id("checkpoint"), choices=sd_models.checkpoint_tiles(), value='', tooltip="switch to another model in the middle of generation")
                ...
                refiner_switch_at = gr.Slider(value=0.8, label="التبديل عند", minimum=0.01,
                                               maximum=1.0, step=0.01, elem_id=self.elem_id("switch_at"), 
                                                                  tooltip="النسبة من خطوات المعالجة التي يتم عندها التبديل إلى نموذج المكرر؛ 1=لا يتم التبديل، 0.5=يتم التبديل في منتصف التوليد"
)
